...
// 静的メンバ変数の実体化
ID3D12Device* ToonGraphicsPipline::sDevice = nullptr;

void ToonGraphicsPipline::SetDevice(ID3D12Device* device) {
	assert(device);
	sDevice = device;
}

void ToonGraphicsPipline::InitializeGraphicsPipeline() {
	CreateState();

	CreatePSO();
}

void ToonGraphicsPipline::CreateState() {
	CreateRootsignature();
	CreateInputLayout();
	CreateBlendState();
...
void ToonGraphicsPipline::CreateRootsignature() {
	HRESULT hr = S_FALSE;
	// Rootsignature設定
	...
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::LIGHTING)].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV; //!< CBVで使う
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::LIGHTING)].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL; //!< Shaderで使う
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::LIGHTING)].Descriptor.ShaderRegister = static_cast<int>(ROOT_PARAMETER_TYP::LIGHTING); //!< レジスタ番号3とバインド

	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::TOONTEXTURE)].ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;// DescriptorTableを使う
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::TOONTEXTURE)].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;// PixelShaderで使う
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::TOONTEXTURE)].DescriptorTable.pDescriptorRanges = &descriptorRange[0];
	rootParameters[static_cast<int>(ROOT_PARAMETER_TYP::TOONTEXTURE)].DescriptorTable.NumDescriptorRanges = 1; // 1つのデスクリプタレンジを利用
...
void ToonGraphicsPipline::CreateInputLayout() {
	//InputLayout
	inputLayout_[0] =
	{// xyz座標(1行で書いたほうが見やすい)
...
void ToonGraphicsPipline::CreateBlendState() {
	// 全ての色要素を書き込む
	blendDesc_.RenderTarget[0].RenderTargetWriteMask = D3D12_COLOR_WRITE_ENABLE_ALL;
}

void ToonGraphicsPipline::CreateRasiterzerState() {
	// 表面（時計回り）を表示しない
...
void ToonGraphicsPipline::CreateDepthStencil() {
	// Depthの機能を有効化する
...
void ToonGraphicsPipline::CreatePSO() {
	HRESULT hr = S_FALSE;
	//PSO生成
...
void ToonGraphicsPipline::DXCCompilerInitialize() {
	HRESULT hr = S_FALSE;
	// dxcCompilerを初期化
...
IDxcBlob* ToonGraphicsPipline::CompileShader(const std::wstring& filePath, const wchar_t* profile, IDxcUtils* dxcUtils, IDxcCompiler3* dxcCompiler, IDxcIncludeHandler* includeHandler) {
